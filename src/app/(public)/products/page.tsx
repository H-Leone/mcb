                                    ...
                                    <div className="flex lg:flex-col gap-2 overflow-x-auto pb-2 lg:pb-0">
                                        {categories.map(c => (
                                            <button
                                                ...
                                                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap lg:text-left ${
                                                    category === c.tag
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                                ...
                                {paginatedProducts.map(p => (
                                    <Link
                                        key={p.id}
                                        href={`/products/${p.id}`}
                                        className="group bg-slate-50 rounded-2xl p-5 border border-slate-200 hover:shadow-2xl transition-all flex flex-col"
                                    >

                                        {/* 🔥 IMAGEM CORRIGIDA */}
                                        <div className="relative aspect-square w-full rounded-xl bg-slate-100 overflow-hidden mb-4">
                                            <Image
                                                src={p.image}
                                                alt={p.name}
                                                fill
                                            ...
                                            <div className="absolute top-3 left-3 px-2 py-1 bg-white/80 rounded text-[9px] font-black text-slate-500">
                                                {p.tag}
                                            </div>
                                        </div>

                                        <h3 className="text-base font-bold text-slate-900 line-clamp-2 min-h-[3.5rem] group-hover:text-blue-600">
                                            {p.name}
                                        </h3>

                                        <div className="mt-auto pt-4 border-t flex items-center justify-between">
                                            <span className="text-blue-600 font-bold text-[10px] uppercase flex items-center gap-2">
